...
async function addExif(webpBuffer) {
  ...
  const json       = {
    ...
    'sticker-pack-name': '⚔️ Yuta Okotsu MD',
    'sticker-pack-publisher': 'DuarteXV',
    'emojis': ['⚔️']
  }
...
export default {
  ...
  async run({ sock, from, msg, usedPrefix, react, reply }) {
    try {
      ...
      if (!targetMsg) return await reply({ text: `❌ Envía o responde una imagen, video (máx 15s) o sticker.\n\n💡 *${usedPrefix}s* con media adjunta.` })
      ...
      const stickerFinal = await addExif(webpBuffer)
